...
namespace Recommender_sys_test
{
    class Program
    {
        ...
        static (IDataView training, IDataView test) LoadData(MLContext mlContext)
        {
            ...
            IDataView testDataView = loader.Load(dbSource1);
        ...
        static void UseModelForSinglePrediction(MLContext mlContext, ITransformer model)
        {
            ...
            {
                Console.WriteLine("Song " + testInput.ID_SONG + " is recommended for user " + testInput.ID_USER);
            }
            else
            {
                Console.WriteLine("Song " + testInput.ID_SONG + " is not recommended for user " + testInput.ID_USER);
            }
        }
